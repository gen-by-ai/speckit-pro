...
Focus: **survive the night + learn from every run.** Closes the remaining open items of the v1.7 reliability audit (IMPROVEMENTS.md P1 #6/#7/#8/#15/#16/#17 + the P0 #3 residual), adds the cross-run analytics layer the telemetry was missing, and lands the audit's "missing test layer": a fake-agent-CLI state-machine suite that exercises every orchestrator terminal path without burning a token.

### Added — survive the night (headless `pro-orchestrate.sh`)
- **Per-call timeouts (P1#7).** Every generator/evaluator/revision invocation runs under `--iteration-timeout` (default 1800s; env `SPECKIT_PRO_ITERATION_TIMEOUT`; `loop.iteration_timeout`). Uses coreutils `timeout`/`gtimeout` when present, otherwise a pure-bash watchdog (detached from the capture pipe — a watcher holding it would block the caller for the full deadline). Timeout ⇒ `ERROR:iteration-timeout-<N>s`, counts toward the breaker. A whole-run `--max-wall-seconds` budget stops cleanly (checkpoint + state + notification, exit 0).
- **No-progress watchdog (P1#6).** The checkbox delta is the one progress signal the agent can't get wrong by accident: `--no-progress-limit` (default 3) consecutive CONTINUE/unknown iterations without a new `[x]` stop the run with a diagnostic instead of burning the whole budget doing nothing.
- **UNKNOWN counts toward the breaker (P1#6).** A crashed CLI, expired auth or rate-limit returns no tag; the old "treating as CONTINUE" (plus a failure-counter reset) defanged the breaker exactly when it was needed most. The worker's documented `MAX_ITERATIONS` tag is also handled explicitly now (clean stop, not "unknown"). Ported to `pro-orchestrate.ps1`.
- **Per-iteration transcripts (P1#8a).** Generator/evaluator/revision stdout+stderr persist to `.knowledge/features/<feature>/logs/iter-N[.eval-rR|-rev-R].log` — the audit trail for "what did it actually do at 3am". Revision output is no longer discarded on the copilot/generic branches (`&>/dev/null` removed).
- **File-based status contract (P1#8b).** The worker writes `{"status":…,"reason":…}` to `<spec-dir>/.pro-status.json` (pro.loop.md protocol addition); the orchestrator consumes it each iteration and prefers it over the stdout tag scrape (BUDGET_STOP always wins). Survives CLI cost-footers, mid-answer tag mentions, and dropped final lines. Stale files are deleted before each call; the stdout tag remains required as the fallback channel.
- **Durable loop state (P1#15).** `.knowledge/features/<feature>/loop-state.json` (iteration, failures, streak, counts, status, run-id) written every iteration and on every terminal path; resume reads it before falling back to the progress.md label regex. The iteration budget is now **relative**: `--max-iterations 8` on a resume at iteration 13 runs 13..20 — the old absolute comparison made exactly that resume a silent no-op.
- **Single-orchestrator lock (P1#16).** noclobber-atomic `.knowledge/features/<feature>/.lock` with PID + staleness detection (dead holder ⇒ takeover with warning; live holder ⇒ abort with guidance; `--force-lock` for confirmed crash leftovers). Released on EXIT.
- **Deferred-blocker journal (P1#17, skip half).** BLOCKED reasons append to `.knowledge/features/<feature>/blocked.md`; the next iteration's prompt carries `blocked-log=<path>` and the loop protocol instructs the worker to pick a different independent work unit instead of re-running into the same wall.
- **Webhook notifications (P1#17, notify half — the long-documented `notify.*` config is finally wired).** Every notable event (circuit_breaker, watchdog_no_progress, eval_hard_fail, budget_stop, wall_clock_stop, terminated, max_iterations, run_complete) is ALWAYS appended to `.knowledge/metrics/notifications.jsonl`; with `notify.webhook_url` (or `SPECKIT_PRO_WEBHOOK_URL` / `--webhook-url`) and the matching gate (`notify.on_failure` / `notify.on_complete`), it's also POSTed as JSON with a Slack-compatible `text` field. Best-effort: never aborts the run.
- **`pro-orchestrate.sh --doctor` (P1#13).** Prints resolved CLI + version + capability set, agent-definition resolution, timeout binary, python3/curl/git state, config cascade, every resolved knob, active `SPECKIT_PRO_*` env overrides, webhook + lock + loop-state status, and a READY/NOT-READY verdict. Answers "will this even start?" before an overnight run. All major knobs now honor `SPECKIT_PRO_*` env overrides (flag > env > default).
- **SIGTERM handling (P1#14 residual).** `kill`/CI-cancel now records a final session entry + notification and exits 143 instead of vanishing; lock release + telemetry finish ride the EXIT trap.

### Added — analytics (the system was data-rich but analysis-poor)
- **`pro-analytics.sh` + `/speckit.pro.analytics`.** Three read-only views over `runs.jsonl` + run manifests + `notifications.jsonl`: **summary** (per-feature rollups: runs/pass-rate/avg-score/iterations/tasks/cost; failure taxonomy counted by class; cost total/per-run/per-completed-task; rule-based recommendations), **failures** (chronological classified failure ledger with pointers to the matching iteration transcripts), **health** (composite 0-100: pass rate 40 + task completion 20 + failure-free rate 20 + score trend 20, letter grade; `--gate N` exits 1 below N — cron-able degradation alarm). `--json`, `--feature`, `--last`, `--write` supported; `SPECKIT_PRO_METRICS_DIR` for hermetic tests. Complements `pro-report.sh aggregate` (trends) with the *why-do-runs-fail* and *what-does-it-cost* layers.
- **Cross-repo insights loop (`export` / `import`).** Pro runs in many consumer repos; learnings should travel back to wherever the extension improves. **`export`** writes one portable, self-contained JSON bundle (`schema: speckit-pro/insights-bundle`, v1): source identity (repo/branch/installed Pro version), the full analytics summary, the recent classified failure ledger, and the improvements-ledger learnings (Promoted + Proposed, raw text) — `--out FILE` (default `.knowledge/metrics/insights-<repo>-<date>.json`), `--stdout`, `--anonymize` (masks repo/branch/feature identity incl. inside recommendations, strips failure details; warns that learnings text ships verbatim); works telemetry-less if the ledger exists. **`import <bundle.json...>`** rolls up any number of bundles: per-source health/pass-rate/cost table, merged failure taxonomy (totals + repos affected), recurring recommendations ranked by repo count, collected learnings with provenance; invalid files are skipped loudly, exit 1 only when nothing valid remains; `--json`, `--write` (→ `insights-imported.md`). **`import --to-ledger`** appends collected learnings to `.knowledge/improvements.md` under `## Proposed` — newest-first, deduped by bold lesson title against the whole ledger, stamped `Imported-from: <repo> (bundle <date>)`; source-promoted entries are demoted to `status: proposed` on arrival, so promotion never crosses repo boundaries un-reviewed (the ledger's human-gate contract holds).

### Fixed
- **Agent definitions resolve again (P0#3 residual).** All four CLI branches (bash) + PowerShell hardcoded `.github/agents/…` — a path that exists in NEITHER the dev repo NOR installed consumers (`.extensionignore` excludes `.github/`), so every headless run drove a missing file. New `resolve_agent_file` search order: `$SPECKIT_PRO_AGENTS_DIR` → `<script-dir>/../../agents` (covers installed AND dev layouts) → `.specify/extensions/pro/agents` → `agents/` → `.github/agents` (legacy). Missing ⇒ explicit `ERROR:agent-definition-missing`, named in `--doctor`. Same cascade applied to `pro-fanout.sh` (scan-worker def — its `agents/` fallback only existed in the dev repo) and `pro-improve-guard.sh` (whose probe gate FAIL-CLOSED on every machine because its evaluator path could never exist).
- **PowerShell claude invocation could never work.** `--system-prompt $agentFile` passed a *path* to a flag that doesn't exist in claude 2.x; now `--append-system-prompt (Get-Content -Raw …)` + `--permission-mode acceptEdits` + `--allowedTools`, matching the bash orchestrator (FR-001 parity). Tag scrape takes the LAST match, not the first (an early protocol mention poisoned it).
- **Malformed evaluator PASS score silently accepted (P1#9 residual).** `PASS:82/100` / `PASS:eighty` survived the old `[[ … ]] 2>/dev/null` arithmetic and were accepted; now validated against `^[0-9]{1,3}$` — malformed ⇒ revision, never acceptance.
- **`detect_agent_cli` stdout-capture bug.** The fallback warning was emitted to stdout *inside* `$(…)`, so the resolved CLI name arrived polluted ("[Pro] ⚠ …\ncopilot") and silently routed real CLIs through the generic branch. Diagnostics now go to stderr.
- **Banner mislabeled** iteration count as task progress; now shows completed/total tasks plus the run-relative budget.
- `agents/speckit.pro.loop.agent.md` shim pointed at `commands/speckit.pro.loop.md` — a file that has never existed (actual: `commands/pro.loop.md`).

### Tests
- **`scripts/tests/orchestrator-smoke.sh` (17 checks)** — the audit's "missing test layer". A scripted fake agent CLI (emits tags, ticks checkboxes, sleeps, writes the status file) drives the real orchestrator through every terminal path in hermetic sandboxes: completion, already-complete precheck, breaker on errors / on UNKNOWN, watchdog, per-call timeout, live-lock refusal, stale-lock takeover, resume relative budget, status-file override, blocked-journal feedback, agent MAX_ITERATIONS tag, run iteration budget, doctor, evaluator invalid-verdict hard-fail, evaluator malformed-score revision, notifications-JSONL validity. Zero tokens burned.
- **`scripts/tests/analytics-smoke.sh` (13 checks)** — hermetic export/import coverage: bundle schema validity, default-file write, anonymization actually masks repo/branch/feature and strips failure details, telemetry-less ledger-only export, two-repo rollup (merged taxonomy + collected learnings), import `--json` shape, `--to-ledger` appends into `## Proposed` only (never `## Promoted`), title-dedupe across re-imports, invalid-bundle handling (mixed → succeed, all-invalid/none → exit 1), flag-value validation (missing/non-numeric values error — never hang, never silently pass), malformed-nested-field bundles skipped without aborting the batch, bold-less learning dedupe, anonymize-with-unmatched-filter masking.

### Fixed — pre-release adversarial gate (17 confirmed findings, every one repro'd before and after)
A 4-lens review workflow (shell portability / python logic / release consistency / orchestrator diff) with adversarial verification ran over the full pending diff before tagging. Every confirmed finding was fixed and re-verified:

- **`pro-analytics.sh` arg parser could hang forever at 100% CPU** when a value-taking flag was the last argument (`shift 2` with one arg left is a non-consuming no-op under `set -uo pipefail`) — worst for the cron-able `health --gate`. Value-taking flags now validate eagerly; non-numeric `--last`/`--gate` error loudly (a typo'd gate used to **silently exit 0** — a dead alarm).
- **`import` hardened against malformed bundles.** Wrong-*typed* nested fields (`"analytics": "oops"`, `"learnings": {"proposed": "a string"}`) crashed the whole batch with a raw traceback — and a learnings *string* would have been iterated per-character into garbage ledger entries. Extraction is now all-or-nothing per bundle: malformed bundles are skipped loudly, valid ones still import, exit 1 only when nothing valid remains.
- **`export --anonymize` leaked the raw `--feature` filter name** in `window.feature`/`feature_filter` when the filter matched no runs; the filter name now always enters the mask map.
- **Ledger dedupe was a no-op for learnings without a bold `**title**`** — repeated imports duplicated them indefinitely. Dedupe now also keys on raw entry head lines.
- **`health` crashed with a TypeError** when 4+ eval scores were non-numeric (foreign/hand-edited telemetry); the trend filter now requires numeric scores.
- **Smoke suites run from a consumer install now** (previously 0/17 and 0/9 in a `.specify/extensions/pro` layout): all three suites resolve the script under test relative to themselves, not the git toplevel, and unset leaked `SPECKIT_PRO_*` env (a leaked `SPECKIT_PRO_METRICS_DIR` caused 3 false FAILs **and wrote a file outside the sandbox**).
- **`loop.iteration_timeout` / `loop.max_wall_seconds` / `loop.no_progress_limit` were documented but dead** — never read from pro-config.yml (the very defect class P1#13 closed). Now wired through `cfg_get` with the documented precedence flag > env > config > default, non-numeric config values warned and ignored, `--doctor` heading updated.
- **`notify_event` emitted invalid JSON when detail carried control characters** (ANSI escapes from agent-CLI stderr) — corrupting `notifications.jsonl` on exactly the failure events it exists to record, and analytics silently dropped those lines. `json_escape` now strips control bytes; detail truncation switched from `head -c` (splits multibyte UTF-8) to character-safe `truncate_chars`.
- **`release_lock` deleted the lock without an ownership check** — after a `--force-lock` steal, the dying original holder removed the *new* holder's lock, re-opening the door to concurrent orchestrators. Release now verifies the lock file's PID is its own.
- **The pure-bash timeout fallback orphaned one full-deadline `sleep` per call** (up to 30 min each on macOS without coreutils). The watcher now polls and self-exits ≤1s after the command finishes; timeout-fires behavior unchanged.
- **PowerShell parity for the v1.24 control-signal protocol** (pinned by a 33-check pwsh harness): the worker's `MAX_ITERATIONS` tag gets an explicit clean-stop case (was: false circuit breaker after 3 "unknown status" retries), bare `BLOCKED` matches with the bash "(no reason given)" fallback, and the `<spec-dir>/.pro-status.json` contract is fully ported (stale-delete before each call, read+validate+prefer over the stdout scrape, BUDGET_STOP-wins, delete after consumption — the file no longer lingers and leaks into checkpoint commits). Also fixed: `Get-TaskCounts`/`Test-AllTasksDone` crashed under `Set-StrictMode` on 0/1 grep matches (`.Count` on unwrapped `Where-Object` output) — every real ps1 run died in iteration 1.
- **Version string aligned**: `extension.yml` `1.24` = CHANGELOG `[1.24]` = tag `v1.24` (was `1.24.0`, which `pro-analytics.sh export` would have propagated cross-repo as a version matching no changelog entry). `IMPROVEMENTS.md` (dev audit register) excluded from consumer installs via `.extensionignore`.
