...
When `knowledge.enabled: true` and `knowledge.prime_before_specify: true` (default), before phase detection:

```
EXECUTE_COMMAND: /pro.knowledge-sync --mode prime --query "<feature-dir> <spec.md H1 if spec exists>"
```

Bootstrap `.knowledge/` automatically when missing and `knowledge.auto_bootstrap: true`. Keep `<pro-knowledge-prime>` in context for contract generation and the loop.

## Phase Detection

Inspect the feature dir and `.knowledge/features/<feature>/` to classify it:

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
SPEC_DIR="$PROJECT_ROOT/specs/<feature-dir>"
FEATURE_KNOWLEDGE_DIR="$PROJECT_ROOT/.knowledge/features/<feature-dir>"
```
...
| `tasks.md` exists, no `<FEATURE_KNOWLEDGE_DIR>` | `tasks-only` | Run Phase 5b (init.sh + AGENT.md), generate sprint-1 contract, start loop. |
| `<FEATURE_KNOWLEDGE_DIR>/contracts/` is empty | `contracts-missing` | Run `/pro.contract`, then start loop. |
| `<FEATURE_KNOWLEDGE_DIR>/init.sh` missing | `no-initializer` | Run Phase 5b only, then start loop. |
...
### Step: Knowledge prime (before contract, if tasks just completed)

When `knowledge.enabled: true` and `knowledge.prime_before_contract: true`:

```
EXECUTE_COMMAND: /pro.knowledge-sync --mode prime --query "<feature-dir> <spec.md H1>"
```
...
1. Derive `<FEATURE_KNOWLEDGE_DIR>` and create directories.
2. Update `.gitignore` (`.knowledge/features/`, optionally `specs/`).
...
If `<FEATURE_KNOWLEDGE_DIR>/AGENT.md` already exists from a previous abandoned attempt, **do not overwrite** — append a `## Pickup Notes — <ISO timestamp>` section noting the resumption.

### Step: Start the loop

The pickup command is the loop driver. Execute iterations per `pro.go.md` **Phase 6** — including **knowledge prime at step 0** of each iteration when `knowledge.prime_each_loop_iteration: true`.

### Step: Post-implement (after loop)

When the loop finishes, run **`pro.go.md` Phase 7** in full (do not skip):
...
Run knowledge-sync only after evaluator **PASS** (same rules as Phase 7d in `pro.go.md`).
...
.specify/extensions/pro/scripts/bash/pro-orchestrate.sh \
  ...
  --spec-dir "$SPEC_DIR" \
  --knowledge-feature-dir "$FEATURE_KNOWLEDGE_DIR" \
  --resume
```
