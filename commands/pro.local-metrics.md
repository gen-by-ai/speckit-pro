...
> **Run-level metrics** (duration, files/lines produced, iterations, eval score, parallelization factor per `/pro.go` run) live in a separate log — `.knowledge/metrics/runs.jsonl`, written by `pro-report.sh` at Phase 8 — with per-worker fan-out events in `.knowledge/metrics/fanout-metrics.jsonl`. As of v1.23 each run record also carries headless-orchestration telemetry rolled up from the per-run manifest (`.knowledge/metrics/runs/<run_id>.json`, accumulated by `pro-report.sh call` / `pro-report.sh phase`): `total_cost_usd`, per-call token counts (`input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_creation_tokens`), `turns`, `session_id` (+ distinct `session_ids`), per-phase wall-clock (`per_phase_durations_s`), and the `human_interventions` / `rework_count` / `circuit_breaker_trips` counts. Fields that were never reported are stored as `null` (rendered "unavailable", never 0). For the cross-run trend dashboard + recommendations, run `pro-report.sh aggregate` — which now also prints the **eval-score trend** (e.g. `72 → 78 → 81 → 65 ⚠ REGRESSION`) and a **regression flag** when the latest scored run drops below the trailing mean (`reporting.regression.*`). Run-level telemetry is exported to an OpenTelemetry collector only when you opt in via `reporting.otel.enabled: true`. This command focuses on local-model quality.
